...
const Nav = () => {
  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 flex justify-between items-center relative z-[101]">
        ...
        {/* MOBILE TOGGLE - Explicitly higher Z-index */}
        <button 
          className="md:hidden text-white relative z-[1000] p-2" 
      ...
      <div 
        className={`fixed inset-0 h-screen w-screen bg-[#0A0A0A] flex flex-col items-center justify-center gap-8 transition-all duration-500 ease-in-out md:hidden 
          ${isOpen 
            ? "opacity-100 visible translate-y-0 z-[999]" 
            : "opacity-0 invisible -translate-y-full z-[-1]"
          }`}
      >
        {navLinks.map((link) => (
          <NavLink 
